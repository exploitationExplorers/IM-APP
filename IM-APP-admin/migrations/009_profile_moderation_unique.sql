...
-- 注意：本项目 migration 无版本记录，每次启动都会重跑全部 .sql，因此本文件必须幂等（可重复执行）。
-- 先清理同 (user_id, field) 的重复记录（保留 id 最大即最新一条），再补唯一约束（已存在则跳过）。
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'uq_profile_mod_user_field'
          AND conrelid = 'profile_moderation_records'::regclass
    ) THEN
