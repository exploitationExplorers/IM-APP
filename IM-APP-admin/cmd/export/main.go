...
var nestedBodyFields = map[string][]fieldSpec{
	...
	"PUT /api/admin/v1/forward-settings settings": {
		{"defaultDailyLimit", "integer", false, "默认每日上限"},
		{"defaultHourlyLimit", "integer", false, "默认每小时上限"},
		{"defaultSingleTargets", "integer", false, "默认单次目标数"},
		{"maxSingleTargets", "integer", false, "单次目标数上限"},
	},
}
