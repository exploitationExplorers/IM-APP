...
const groups = computed<FavoriteGroup[]>(() => {
  ...
  for (const item of favorites.value) {
    const title =
      chatStore.conversations.find((c) => c.id === item.conversationId)?.title ||
      conversationTitleOf(item.conversationId)
...
async function loadFavorites(type: FavoriteListType, reset = false) {
  ...
  loading.value = true
  try {
    const items = await fetchFavorites({ type, page: currentPage, size: PAGE_SIZE })
...
function eventPoint(event: unknown) {
  ...
  const detail = typeof raw.detail === 'object' && raw.detail ? raw.detail : undefined
  return {
...
function updateMenuPosition(item: FavoriteItem, event: unknown) {
  ...
  const leftRpx =
    rightSpaceRpx >= MENU_WIDTH_RPX
      ...
      : Math.max(clickXRpx - MENU_WIDTH_RPX - MENU_OFFSET_RPX, VIEWPORT_PADDING_RPX)
...
<template>
  <view class="page">
    ...
    >
      <view v-for="group in groups" :key="group.key" class="group">
        ...
        >
          ...
          <text v-if="item.type === 'text' || item.type === 'emoji'" class="card-text">
            {{ displayText(item) }}
          </text>
          <image
    ...
    <view v-if="activeMenuItem && menuStyle.top" class="menu-mask" @click="closeMenu">
      <view class="menu" :style="menuStyle" @click.stop>
        <view class="menu-item" @click="handleAction('forward', activeMenuItem)">转发</view>
        <view
          v-if="activeMenuItem.type === 'text' || activeMenuItem.type === 'emoji'"
          class="menu-item"
